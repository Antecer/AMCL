...
namespace FTPGet
{
    ...
    public class FTPClient
    {
        ...
        public string[] Dir(string strMask)
        {
            ...
            for (int i = 0; i < strsFileList.Length; i++) strsFileList[i] = strsFileList[i].Trim();
            return strsFileList;
        }
        ...
        public string[] FileDir()
        {
            ...
            char[] seperator = { '\n' };
            string[] FileList = strMsg.Split(seperator);
            socketData.Close(); //数据socket关闭时也会有返回码
            if (iReplyCode != 226)
            {
                ReadReply();
        ...
        /// 获取文件夹列表
        /// </summary>
        /// <returns>返回文件夹名列表(数组最后一个值有可能是"")</returns>
        public string[] FolderDir()
        {
            string[] AllList = Dir("");     //获取目录列表
            string[] FileList = Dir("*.*"); //获取文件列表
            string strFolderList = "";
            for (int i = 0; i < AllList.Length; i++)    //遍历目录列表，剔除文件列表，得到文件夹列表
            {
                if (FileList.Contains(AllList[i])) continue;
                strFolderList += AllList[i] + "|";      //将获得的文件夹名写入字符串并添加“|”分隔符
            }
            strFolderList = strFolderList.TrimEnd('|'); //删除字符串结尾多余的分隔符“|”
            string[] FolderList = strFolderList.Split('|');
            return FolderList;
        }
        ...
        /// </summary>
        /// <param name="strFileName">文件名</param>
        /// <returns>返回协调世界时</returns>
        public string GetFileTime(string strFileName)
        {
            if (!bConnected)
            {
                Connect();
            }
            SendCommand("MDTM " + Path.GetFileName(strFileName));
            if (iReplyCode == 213)
            {
                return strReply.Substring(4).Trim();
            }
            else
            {
                throw new IOException(strReply.Substring(4));
        ...
        public void GetFile(string strRemoteFileName, string strFolder, string strLocalFileName)
        {
            ...
            if (strLocalFileName.Trim().Equals("")) strLocalFileName = strRemoteFileName.Trim();
            FileStream output = new FileStream(strFolder + "\\" + strLocalFileName, FileMode.Create);
            Socket socketData = CreateDataSocket();
            SendCommand("RETR " + strRemoteFileName.Trim());
            if (!(iReplyCode == 150 || iReplyCode == 125
            || iReplyCode == 226 || iReplyCode == 250))
            {
                throw new IOException(strReply.Substring(4));
        ...
        /// <param name="strFolder">本地目录(不得以\结束)</param>
        /// <param name="strLocalFileName">保存在本地时的文件名</param>
        /// <param name="size">文件开始下载的位置</param>
